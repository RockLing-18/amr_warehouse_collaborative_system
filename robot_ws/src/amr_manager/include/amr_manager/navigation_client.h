...
// 所有接口使用相对名，robot namespace（如 /robot_01）
...
public:
    ...
    using FeedbackCallback = std::function<void(const std::shared_ptr<const AmrNavigateToPose::Feedback> feedback)>;
    using ResultCallback = std::function<void(const GoalHandle::WrappedResult & result)>;
    using PoseQueryCallback = std::function<void(bool success, double x, double y, double yaw)>;
...
private:
    ...
    rclcpp_action::Client<AmrNavigateToPose>::SharedPtr m_action_client;
    rclcpp::Client<GetRobotPose>::SharedPtr m_pose_service_client;
    rclcpp::Subscription<RobotPose>::SharedPtr m_pose_sub;

    // 当前目标句柄：取消 / 状态查询用（每次发送后更新）
    GoalHandle::SharedPtr m_current_goal_handle;

    mutable std::mutex m_pose_mutex;
    RobotPose::SharedPtr m_latest_pose;

    FeedbackCallback m_feedback_cb;
    ResultCallback m_result_cb;
};
