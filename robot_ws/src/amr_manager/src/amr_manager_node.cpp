...
//   topic  : robot_pose
//
// 启动（预留 robot namespace）:
//   ros2 run amr_manager amr_manager_node --ros-args -r __ns:=/robot_01
...
public:
  ...
  : Node("amr_manager")
  {
    ...
    // 相对名，robot namespace（如 /robot_01）自动生效
    robot_status_pub_ = this->create_publisher<RobotStatus>("robot_status", 10);
    task_status_pub_ = this->create_publisher<TaskStatus>("task_status", 10);
  }

  void init()
  {
    nav_client_ = std::make_shared<NavigationClient>(shared_from_this());
    ...
    nav_client_->setResultCallback(
      [this](const NavigationClient::GoalHandle::WrappedResult & result)
      {
        // TODO: 任务状态推进（SUCCEEDED / FAILED / CANCELED）
        (void)result;
      });
...
private:
  void publishStatus()
  {
    ...
    auto pose = nav_client_->getLatestPose();
    ...
    // TODO: 发布 task_status（当前任务快照，无任务时跳过）
    // TaskStatus ts;
    // ts.task_id = ...;
    // ...
    // task_status_pub_->publish(ts);
  }
